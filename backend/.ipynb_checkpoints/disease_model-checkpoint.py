...
class DiseaseModel:
    ...
    def load_xgboost(self, model_path):
        try:
            ...
            # Fix for XGBClassifier not having n_classes_ set after load_model
            if not hasattr(self.model, 'n_classes_') or self.model.n_classes_ is None:
                from sklearn.preprocessing import LabelEncoder
                le = LabelEncoder()
                le.fit(self.diseases)
                self.model._le = le
                self.model.n_classes_ = len(self.diseases)
                print(f"Manually set n_classes_ to {self.model.n_classes_} and _le")
