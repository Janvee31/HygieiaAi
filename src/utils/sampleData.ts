...
export const getRandomLungData = () => {
  ...
  const generateLungProfile = () => {
    ...
    if (isPositiveProfile) {
      ...
      return {
        age: formatNumber(getRandomNumber(55, 80)),
        gender: formatNumber(getRandomNumber(0, 1)), // Random gender
        smoking: formatNumber(1), // Smoker
        yellow_fingers: formatNumber(getRandomNumber(1, 2)), // Yellow fingers likely
        anxiety: formatNumber(getRandomNumber(1, 2)), // Anxiety likely
        peer_pressure: formatNumber(getRandomNumber(0, 1)), // Random peer pressure
        chronic_disease: formatNumber(getRandomNumber(1, 2)), // Chronic disease likely
        fatigue: formatNumber(getRandomNumber(1, 2)), // Fatigue likely
        allergy: formatNumber(getRandomNumber(0, 1)), // Random allergy
        wheezing: formatNumber(getRandomNumber(1, 2)), // Wheezing likely
        alcohol_consuming: formatNumber(getRandomNumber(0, 2)), // Random alcohol consumption
        coughing: formatNumber(getRandomNumber(1, 2)), // Coughing likely
        shortness_of_breath: formatNumber(getRandomNumber(1, 2)), // Shortness of breath likely
        swallowing_difficulty: formatNumber(getRandomNumber(1, 2)), // Swallowing difficulty likely
        chest_pain: formatNumber(getRandomNumber(1, 2)) // Chest pain likely
      };
    } else {
      // Negative lung cancer profile (healthy values)
      return {
        age: formatNumber(getRandomNumber(20, 55)),
        gender: formatNumber(getRandomNumber(0, 1)), // Random gender
        smoking: formatNumber(0), // Non-smoker
        yellow_fingers: formatNumber(0), // No yellow fingers
        anxiety: formatNumber(getRandomNumber(0, 1)), // Random anxiety
        peer_pressure: formatNumber(getRandomNumber(0, 1)), // Random peer pressure
        chronic_disease: formatNumber(0), // No chronic disease
        fatigue: formatNumber(getRandomNumber(0, 1)), // Random fatigue
        allergy: formatNumber(getRandomNumber(0, 1)), // Random allergy
        wheezing: formatNumber(0), // No wheezing
        alcohol_consuming: formatNumber(getRandomNumber(0, 1)), // Random alcohol consumption
        coughing: formatNumber(0), // No coughing
        shortness_of_breath: formatNumber(0), // No shortness of breath
        swallowing_difficulty: formatNumber(0), // No swallowing difficulty
        chest_pain: formatNumber(0) // No chest pain
      };
    }
  };
...
// Random sample data for Stroke Risk - Based on Kaggle stroke dataset
export const getRandomStrokeData = () => {
  // Create a function to generate high and low stroke risk profiles
  const generateStrokeProfile = () => {
    // 40% chance of generating a high stroke risk profile
    const isHighRiskProfile = Math.random() < 0.4;
    
    if (isHighRiskProfile) {
      // High stroke risk profile
      return {
        age: formatNumber(getRandomNumber(60, 85)),
        gender: getRandomBoolean() ? 'male' : 'female', // Random gender
        hypertension: getRandomBoolean(0.7) ? 'yes' : 'no', // 70% chance of hypertension
        heart_disease: getRandomBoolean(0.6) ? 'yes' : 'no', // 60% chance of heart disease
        ever_married: 'yes', // Likely married
        work_type: ['private', 'self-employed', 'govt_job'][getRandomNumber(0, 2)],
        residence_type: getRandomBoolean() ? 'urban' : 'rural',
        avg_glucose_level: formatNumber(getRandomNumber(120, 270, 1)), // Higher glucose
        bmi: formatNumber(getRandomNumber(28, 40, 1)), // Higher BMI
        smoking_status: ['formerly_smoked', 'smokes', 'never_smoked'][getRandomNumber(0, 2)]
      };
    } else {
      // Low stroke risk profile
      return {
        age: formatNumber(getRandomNumber(25, 55)),
        gender: getRandomBoolean() ? 'male' : 'female', // Random gender
        hypertension: getRandomBoolean(0.2) ? 'yes' : 'no', // 20% chance of hypertension
        heart_disease: getRandomBoolean(0.1) ? 'yes' : 'no', // 10% chance of heart disease
        ever_married: getRandomBoolean() ? 'yes' : 'no', // Random marital status
        work_type: ['private', 'self-employed', 'govt_job', 'children', 'never_worked'][getRandomNumber(0, 4)],
        residence_type: getRandomBoolean() ? 'urban' : 'rural',
        avg_glucose_level: formatNumber(getRandomNumber(70, 110, 1)), // Normal glucose
        bmi: formatNumber(getRandomNumber(18.5, 27.9, 1)), // Normal BMI
        smoking_status: ['never_smoked', 'unknown'][getRandomNumber(0, 1)]
      };
    }
  };
  
  return generateStrokeProfile();
};

// Random sample data for Thyroid Disease - Based on thyroid dataset
export const getRandomThyroidData = () => {
  // Create a function to generate thyroid disorder and normal profiles
  const generateThyroidProfile = () => {
    // 40% chance of generating a thyroid disorder profile
    const hasThyroidDisorder = Math.random() < 0.4;
    
    // Randomly select between hypothyroidism and hyperthyroidism for disorder cases
    const isHypothyroidism = getRandomBoolean(0.7); // 70% of disorders are hypothyroidism
    
    if (hasThyroidDisorder) {
      if (isHypothyroidism) {
        // Hypothyroidism profile
        return {
          age: formatNumber(getRandomNumber(35, 75)),
          sex: getRandomBoolean(0.7) ? 'female' : 'male', // More common in females
          tsh: formatNumber(getRandomNumber(4.5, 15.0, 2)), // Elevated TSH
          t3: formatNumber(getRandomNumber(0.5, 1.2, 2)), // Low or normal T3
          t4: formatNumber(getRandomNumber(0.5, 4.0, 2)), // Low T4
          family_history: getRandomBoolean(0.6) ? 'yes' : 'no' // 60% chance of family history
        };
      } else {
        // Hyperthyroidism profile
        return {
          age: formatNumber(getRandomNumber(30, 60)),
          sex: getRandomBoolean(0.7) ? 'female' : 'male', // More common in females
          tsh: formatNumber(getRandomNumber(0.01, 0.4, 2)), // Low TSH
          t3: formatNumber(getRandomNumber(1.8, 4.5, 2)), // Elevated T3
          t4: formatNumber(getRandomNumber(12.0, 25.0, 2)), // Elevated T4
          family_history: getRandomBoolean(0.5) ? 'yes' : 'no' // 50% chance of family history
        };
      }
    } else {
      // Normal thyroid profile
      return {
        age: formatNumber(getRandomNumber(20, 65)),
        sex: getRandomBoolean() ? 'female' : 'male', // Random gender
        tsh: formatNumber(getRandomNumber(0.4, 4.5, 2)), // Normal TSH
        t3: formatNumber(getRandomNumber(1.2, 1.8, 2)), // Normal T3
        t4: formatNumber(getRandomNumber(4.0, 12.0, 2)), // Normal T4
        family_history: getRandomBoolean(0.2) ? 'yes' : 'no' // 20% chance of family history
      };
    }
  };
  
  return generateThyroidProfile();
};
