  ...
  useEffect(() => {
    const fetchNotebookContent = async () => {
      try {
        ...
        // Handle both slash and backslash paths, or just filenames
        const notebookName = notebookPath.includes('/') || notebookPath.includes('\\')
          ? notebookPath.split(/[\/\\]/).pop()?.replace('.ipynb', '')
          : notebookPath.replace('.ipynb', '');
        
        // Fetch the notebook file using the new API route
        const response = await fetch(`/api/notebooks?name=${notebookName}`);
        if (!response.ok) {
