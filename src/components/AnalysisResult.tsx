...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ analysis, diseaseType = 'medical', title, result }) => {
  ...
  const getSimplifiedDisease = () => {
    ...
    if (displayInfo.disease.split(' ').length <= 2) {
      return displayInfo.disease;
    }
    
    // Check for common disease patterns
    if (displayInfo.disease.toLowerCase().includes('herpes zoster') || 
        displayInfo.disease.toLowerCase().includes('shingles')) {
      return 'Shingles';
    }
    
    if (displayInfo.disease.toLowerCase().includes('herpes simplex')) {
      return 'Herpes';
    }
    
    if (displayInfo.disease.toLowerCase().includes('carcinoma') || 
        displayInfo.disease.toLowerCase().includes('cancer')) {
      return 'Cancer';
    }
    
    if (displayInfo.disease.toLowerCase().includes('pneumonia')) {
      return 'Pneumonia';
    }
    
    if (displayInfo.disease.toLowerCase().includes('dermatitis')) {
      return 'Dermatitis';
    }
    
    if (displayInfo.disease.toLowerCase().includes('melanoma')) {
      return 'Melanoma';
    }
    
    if (displayInfo.disease.toLowerCase().includes('tuberculosis') || 
        displayInfo.disease.toLowerCase().includes('tb')) {
      return 'Tuberculosis';
    }
    
    if (displayInfo.disease.toLowerCase().includes('fracture')) {
      return 'Fracture';
    }
    
    if (displayInfo.disease.toLowerCase().includes('visual characteristics')) {
      return 'Findings';
    }
    
    // Default: take first two words
    const words = displayInfo.disease.split(' ');
  ...
  const getSimplifiedRisk = () => {
    ...
    if (displayInfo.risk) {
      if (displayInfo.risk.toLowerCase().includes('low') || 
          displayInfo.risk.toLowerCase().includes('minimal') || 
          displayInfo.risk.toLowerCase().includes('mild')) {
        return 'Low';
      }
      
      if (displayInfo.risk.toLowerCase().includes('moderate') || 
          displayInfo.risk.toLowerCase().includes('medium')) {
        return 'Medium';
      }
      
      if (displayInfo.risk.toLowerCase().includes('high') || 
          displayInfo.risk.toLowerCase().includes('severe') || 
          displayInfo.risk.toLowerCase().includes('critical')) {
        return 'High';
      }
      
      if (displayInfo.risk.toLowerCase().includes('postmi')) {
        return 'High';
      }
    }
  ...
  return (
    <div className="flex flex-col h-full max-h-[900px] overflow-y-auto pr-2 custom-scrollbar">
      ...
      {isUnrelated && (
        <div className="glass p-4 rounded-xl mb-4 border border-yellow-500/30 bg-yellow-500/10">
          <div className="flex gap-2">
            ...
            <div>
              <h3 className="font-medium text-yellow-500">Unrelated Image Detected</h3>
              <p className="text-white/70 text-sm">
                The uploaded image doesn't appear to be related to {diseaseTypeLabel}. 
                For accurate analysis, please upload a relevant medical image.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div className="glass p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <p className="text-2xl font-bold text-white">
                {displayInfo.probability ? `${displayInfo.probability}%` : 'N/A'}
              </p>
            </div>
            ...
            <div className="glass p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <p className={`text-2xl font-bold ${getRiskColor(displayInfo.risk, displayInfo.riskLevel)}`}>
                {getSimplifiedRisk()}
              </p>
            </div>
          </div>
