...
export async function POST(request: Request) {
  try {
    ...
    const { data: appointment, error: appointmentError } = await supabaseClient
      .from('appointments')
      .select('id, status, cancelled, doctor_id, appointment_date, appointment_time')
      .eq('id', appointmentId)
      .single();
    ...
    // Also update the doctor's slots_booked to free up the slot
    try {
      // Get the doctor's current booked slots
      ...
      if (!doctorError && doctor) {
        ...
        // Remove the cancelled appointment time from the booked slots
        if (slotsBooked[date] && slotsBooked[date].includes(time)) {
          slotsBooked[date] = slotsBooked[date].filter((slot: string) => slot !== time);
      ...
      console.error('Error updating doctor slots:', slotError);
